...
func TestCommandExecutionP2P(t *testing.T) {
	...
	cmdServer := exec.Command("./test-command-binary", "server", "-passphrase", passphrase, "-command", "echo 'starting command'; cat -; echo 'error log' >&2", "-log-level", "debug")
	...
	// Start client
	cmdClient := exec.Command("./test-command-binary", "client", "-passphrase", passphrase, "-log-level", "debug")
