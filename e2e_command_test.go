...
func TestCommandExecutionP2P(t *testing.T) {
	cmdBuild := exec.Command("go", "build", "-o", "test-command-binary", "./cmd/syncthing-socket")
	...
	cmdServer := exec.Command("./test-command-binary", "server", "--seed", passphrase, "--command", "echo 'starting command'; cat -; echo 'error log' >&2", "--direct-port", "22003", "--discovery", "", "--relay", "", "--log-level", "debug", "--log-format", "text")
	...
	cmdClient := exec.Command("./test-command-binary", "client", "--seed", passphrase, "--relay", "tcp://127.0.0.1:22003", "--discovery", "", "--log-level", "debug", "--log-format", "text")
