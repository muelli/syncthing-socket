...
- `--seed <secret>`: Deterministically generates a secure TLS certificate and Device ID from a secret password, so you don't need to copy-paste long Device IDs.
...
- `--seed <secret>`: Use the same passphrase as the server to auto-discover and connect without needing the Server Device ID!
...
1. **On the Server:**
```bash
./syncthing-socket server --seed "my-secret" --shell
```
2. **On the Client:**
```bash
./syncthing-socket client --seed "my-secret" --shell
```
...
**On the Server:**
```bash
./syncthing-socket server --seed "my-secret" --command "echo 'Processing data...'; cat - | grep error; echo 'Done!'"
```
**On the Client:**
```bash
echo "log data with error" | ./syncthing-socket client --seed "my-secret"
```
...
./syncthing-socket server --seed my-secret --reverse-forward 0.0.0.0:8080
# Or: ./syncthing-socket server --seed my-secret --reverse-forward /tmp/remote.sock
```

**2. On the Client (Local Machine):**
```bash
# Forward incoming reverse tunnel traffic to local port 8000 or a UNIX socket
./syncthing-socket client --seed my-secret --reverse-forward 127.0.0.1:8000
# Or: ./syncthing-socket client --seed my-secret --reverse-forward /var/run/docker.sock
```
